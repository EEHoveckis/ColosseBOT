	...
	infractionID(database).then(infractionID => {
		countActiveWarns(database, message.author.id, message.guild.id).then(activeWarns => {
			switch (guildData.punishmentLevel) {
				case 1:
					addWarn(database, message, "mention", infractionID);
					incrementWarns(database, message);
					antiSpamEmbeds(client, message, guildData, userData, "warn", { reason: "mention", infractionID: infractionID });
					break;
				case 2:
					if (activeWarns < 2) {
						addWarn(database, message, "mention", infractionID);
						incrementWarns(database, message);
						antiSpamEmbeds(client, message, guildData, userData, "warnNF", { activeWarns: activeWarns + 1, reason: "mention", infractionID: infractionID });
					} else if (activeWarns == 2) {
						addWarn(database, message, "mention", infractionID);
						incrementWarns(database, message);
						antiSpamEmbeds(client, message, guildData, userData, "warnF", { reason: "mention", infractionID: infractionID });
					} else if (activeWarns == 3) {
						if (message.member.roles.cache.has(guildData.muteRole)) {
							message.delete();
						} else {
							message.member.roles.add(guildData.muteRole);
							getReason(client, database, guildData, userData, message, { translate: "GUILD", lastReason: "mention" }).then(reason => {
								getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "mention" }).then(reason2 => {
									getReason(client, database, guildData, userData, message, { translate: "NT", lastReason: "mention" }).then(muteReason => {
				...
				case 3:
					if (activeWarns < 2) {
						...
						getReason(client, database, guildData, userData, message, { translate: "GUILD", lastReason: "mention" }).then(reason => {
							getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "mention" }).then(reason2 => {
								getReason(client, database, guildData, userData, message, { translate: "NT", lastReason: "mention" }).then(kickReason => {
				...
				case 4:
					if (activeWarns < 2) {
						...
						getReason(client, database, guildData, userData, message, { translate: "GUILD", lastReason: "mention" }).then(reason => {
							getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "mention" }).then(reason2 => {
								getReason(client, database, guildData, userData, message, { translate: "NT", lastReason: "mention" }).then(banReason => {
