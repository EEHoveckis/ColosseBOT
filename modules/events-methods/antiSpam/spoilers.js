	...
	infractionID(database).then(infractionID => {
		countActiveWarns(database, message.author.id, message.guild.id).then(activeWarns => {
			switch (guildData.punishmentLevel) {
				case 1:
					addWarn(database, message, "spoilers", infractionID);
					incrementWarns(database, message);
					antiSpamEmbeds(client, message, guildData, userData, "warn", { reason: "spoilers", infractionID: infractionID });
					break;
				case 2:
					if (activeWarns < 2) {
						addWarn(database, message, "spoilers", infractionID);
						incrementWarns(database, message);
						antiSpamEmbeds(client, message, guildData, userData, "warnNF", { activeWarns: activeWarns + 1, reason: "spoilers", infractionID: infractionID });
					} else if (activeWarns == 2) {
						addWarn(database, message, "spoilers", infractionID);
						incrementWarns(database, message);
						antiSpamEmbeds(client, message, guildData, userData, "warnF", { reason: "spoilers", infractionID: infractionID });
					} else if (activeWarns == 3) {
						if (message.member.roles.cache.has(guildData.muteRole)) {
							message.delete();
						} else {
							message.member.roles.add(guildData.muteRole);
							getReason(client, database, guildData, userData, message, { translate: "GUILD", lastReason: "spoilers" }).then(reason => {
								getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "spoilers" }).then(reason2 => {
									getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "spoilers" }).then(muteReason => {
				...
				case 3:
					if (activeWarns < 2) {
						...
						getReason(client, database, guildData, userData, message, { translate: "GUILD", lastReason: "spoilers" }).then(reason => {
							getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "spoilers" }).then(reason2 => {
								getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "spoilers" }).then(kickReason => {
				...
				case 4:
					if (activeWarns < 2) {
						...
						getReason(client, database, guildData, userData, message, { translate: "GUILD", lastReason: "spoilers" }).then(reason => {
							getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "spoilers" }).then(reason2 => {
								getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "spoilers" }).then(banReason => {
