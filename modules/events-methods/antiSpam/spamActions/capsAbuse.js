		...
		countActiveWarns(database, message.author.id, message.guild.id).then(activeWarns => {
			switch (guildData.punishmentLevel) {
				case 1:
					...
					antiSpamEmbeds(client, message, guildData, userData, "warn", { reason: "caps", infractionID: infractionID });
					break;
				case 2:
					if (activeWarns < 2) {
						addWarn(database, message, "caps", infractionID);
						incrementWarns(database, message);
						antiSpamEmbeds(client, message, guildData, userData, "warnNF", { activeWarns: activeWarns + 1, reason: "caps", infractionID: infractionID });
					} else if (activeWarns == 2) {
						addWarn(database, message, "caps", infractionID);
						incrementWarns(database, message);
						antiSpamEmbeds(client, message, guildData, userData, "warnF", { reason: "caps", infractionID: infractionID });
					} else if (activeWarns == 3) {
						if (message.member.roles.cache.has(guildData.muteRole)) {
							message.delete();
						} else {
							message.member.roles.add(guildData.muteRole);
							getReason(client, database, guildData, userData, message, { translate: "GUILD", lastReason: "caps" }).then(reason => {
								getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "caps" }).then(reason2 => {
									getReason(client, database, guildData, userData, message, { translate: "NT", lastReason: "caps" }).then(muteReason => {
				...
				case 3:
					if (activeWarns < 2) {
						...
						getReason(client, database, guildData, userData, message, { translate: "GUILD", lastReason: "caps" }).then(reason => {
							getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "caps" }).then(reason2 => {
								getReason(client, database, guildData, userData, message, { translate: "NT", lastReason: "caps" }).then(kickReason => {
				...
				case 4:
					if (activeWarns < 2) {
						...
						getReason(client, database, guildData, userData, message, { translate: "GUILD", lastReason: "caps" }).then(reason => {
							getReason(client, database, guildData, userData, message, { translate: "USER", lastReason: "caps" }).then(reason2 => {
								getReason(client, database, guildData, userData, message, { translate: "NT", lastReason: "caps" }).then(banReason => {
